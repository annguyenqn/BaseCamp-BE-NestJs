...
describe('AdminController', () => {
  ...
  it('should return View list of companies', async () => {
    ...
    mockAdminService.getListOfCompaniesWithCounts.mockResolvedValue(
      expectedResult,
    );
  ...
  it('should throw NotFoundException when companies is not found', async () => {
    mockAdminService.getListOfCompaniesWithCounts.mockRejectedValue(
  ...
  it('should change role to admin the user when user is found', async () => {
    ...
    const user = {
      ...
      passwordHash:
        '$2b$10$2Ws4Ryl8VXI7leSfEA1U9eTF6hcsB80Jv.i/8N/XXHy5tWgLl6w7q',
      firstName: 'Regular',
      lastName: 'User',
      status: 'active',
      companyId: 5,
    ...
    const expectResult = {
      ...
      email: 'demouser213@example.com',
      passwordHash:
        '$2b$10$tcylZ6OAkgO0gv5/epwvKOgMapw5zQ7tiGAhPIR..3lb4Zpi2xzPa',
      firstName: 'Regular',
      lastName: 'User',
      status: 'active',
      companyId: 5,
    ...
    jest
      .spyOn(mockAdminService, 'changeRoleToAdmin')
      .mockResolvedValue(expectResult);

    await expect(mockAdminService.changeRoleToAdmin(userId)).resolves.toEqual(
      expectResult,
    );
  });

  it('shold throw ConflictException if user is already admin', async () => {
    mockAdminService.changeRoleToAdmin.mockRejectedValue(
  ...
  it('should activate an agent', async () => {
    ...
    jest
      .spyOn(mockAdminService, 'toggleAgentStatusForCompany')
    ...
    const result = await mockAdminService.toggleAgentStatusForCompany(
      agentId,
      isActive,
    );
  ...
  it('should throw ConflictException if agent is already active', async () => {
    mockAdminService.toggleAgentStatusForCompany.mockRejectedValueOnce(
      new ConflictException('User is already Active'),
    );
    const agentId = 3;
    const isActive = true;
    await expect(
      mockAdminService.toggleAgentStatusForCompany(agentId, isActive),
  ...
  it('should call create company and return the result', async () => {
    ...
    await expect(
      adminController.createCompanty(createCompanyDto),
    ).resolves.toEqual(expectedResult);

    expect(mockAdminService.createCompany).toHaveBeenCalledWith(
      createCompanyDto,
    );
  });
